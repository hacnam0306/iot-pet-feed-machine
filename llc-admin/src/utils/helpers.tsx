...
export const generateHash = (email: string) => {
  return md5(
    email +
      `${process.env.REACT_APP_SECRET_KEY_SEND_GMAIL}` +
      moment().format('DD/MM/YYYY')
  )
}

export const genderMessageFromApi = (
  code: ClientStatusCode | string | number
) => {
  let message
  switch (code) {
...
export const getMomentFromTimer = (
  ...
  crrDate.setHours(hours)
  crrDate.setMinutes(minutes)
  return moment(crrDate, format)
}
